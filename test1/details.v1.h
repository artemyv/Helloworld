...
decltype(auto)
operator<<(forwarder<F> &&ap, T &&t)
{
    return make_forwarder(
        [ap = std::forward<forwarder<F>>(ap), t = std::forward<T>(t)](auto &&... a) { return ap(t, a...); });
}
...
class LogStream
{
  public:
    ...
    template <typename F>
    bool
    operator==(const forwarder<F> &line) noexcept
    {
        return line();
    }
};
...
#define LOG_STREAM_CALL_V1(logger, level)                                                                              \
    logger && logger->should_log(level) &&                                                                             \
        detail::v1::LogStream() == detail::v1::make_forwarder([a_logger = logger, a_level = level](auto &&... a) {     \
            a_logger->log(spdlog::source_loc{__FILE__, __LINE__, SPDLOG_FUNCTION},                                     \
                          a_level,                                                                                     \
                          detail::v1::make_pattern<sizeof...(a)>(),                                                    \
